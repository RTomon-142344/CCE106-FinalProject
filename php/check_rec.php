...
error_log("check_rec.php - Session data: " . print_r($_SESSION, true));

// Pass whether the user is admin (dept == 1)
$isAdmin = (isset($_SESSION['dept']) && $_SESSION['dept'] == 1);

error_log("check_rec.php - Is admin: " . ($isAdmin ? 'Yes' : 'No'));
...
// Also pass the current user's email for debugging
if (isset($_SESSION['user'])) {
    echo "<script>const currentUserEmail = '" . htmlspecialchars($_SESSION['user']) . "';</script>";
}
?>